...
import { createEquipmentState, getSelectedEquipmentPayload } from './equipment-state.js';

import { createEquipmentUI } from './equipment-ui.js';
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const eventService = createCalendarEventService(state, config);
    const equipmentState = createEquipmentState();
    const equipmentApi = createEquipmentApi(config);
    const equipmentListElement = document.getElementById('equipment-list');
    const equipmentTotalContainer = document.getElementById('equipment-total-container');
    ...
    const equipmentUI =
        equipmentListElement && equipmentTotalContainer && equipmentTotalDisplay
            ? createEquipmentUI(
                  equipmentState,
                  {
    ...
    async function loadEquipmentsForZone(zoneId) {
        console.log('[reservation] Chargement équipements', {
            zoneId,
            selection: state.currentSelection
        });

        if (!equipmentUI || !equipmentListElement) {
            console.warn('[reservation] Interface équipements introuvable');
            return;
        }
        ...
        const selectedStartDate = selection.startDate ?? selection.start;
        const selectedEndDate = selection.endDate ?? selection.end ?? selectedStartDate;
        const startTime = selection.startTime;
        const endTime = selection.endTime;

        if (!selectedStartDate || !selectedEndDate || !startTime || !endTime) {
            console.warn('[reservation] Période incomplète', {
                selectedStartDate,
                selectedEndDate,
                startTime,
                endTime,
        ...
        const startDate = String(selectedStartDate).includes('T')
            ? selectedStartDate
            : `${selectedStartDate}T${startTime}:00`;

        const endDate = String(selectedEndDate).includes('T')
            ? selectedEndDate
        ...
        try {
            ...
            console.log('[reservation] Requête équipements', {
                zoneId,
                startDate,
                endDate
            });

            const equipments = await equipmentApi.getForZone(zoneId, startDate, endDate);

            console.log('[reservation] Équipements reçus', equipments);
            equipmentUI.render(Array.isArray(equipments) ? equipments : []);
        } catch (error) {
            console.error('[reservation] Erreur équipements :', error);
            equipmentListElement.innerHTML = `
    ...
    const zoneTomSelect = new TomSelect(zoneSelectElement, {
        valueField: 'id',
        labelField: 'name',
        searchField: 'name',
        ...
        onChange: async (zoneId) => {
            ...
            try {
                state.currentZonePricings = await api.getPricings(zoneId);
            } catch (error) {
                console.error('[reservation] Erreur tarifs :', error);
                state.currentZonePricings = {};
            }
    ...
    async function loadZonesForFacility(facilityId) {
        ...
        try {
            ...
            if (!zones.length) {
                zoneTomSelect.settings.placeholder = config.texts.noZoneAvailable;
                zoneTomSelect.input.placeholder = config.texts.noZoneAvailable;
            ...
            console.error('[reservation] Erreur zones :', error);
            zoneTomSelect.settings.placeholder = config.texts.noZoneAvailable;
            zoneTomSelect.input.placeholder = config.texts.noZoneAvailable;
            zoneTomSelect.updatePlaceholder();
        }
    ...
    function updateBookingModeLinks() {
        bookingModeLinks.forEach((button) => {
            const isActive = button.dataset.mode === state.bookingMode;
    ...
    calendar = createCalendar({
        ...
        onSelectionChange: async (selection) => {
            console.log('[reservation] Sélection reçue', selection);
            state.currentSelection = selection;
            await loadEquipmentsForZone(state.activeZoneId);
        }
    });
    ...
    bookingModeLinks.forEach((button) => {
        button.addEventListener('click', () => {
            ...
            if (selectedMode !== 'hour' && selectedMode !== 'period') {
                return;
            }
    ...
    if (initialActiveTab) {
        ...
        await loadZonesForFacility(initialActiveTab.dataset.location);
    }
    ...
    submitButton?.addEventListener('click', async () => {
        ...
        try {
            ...
            const { response, result } = await api.createBooking(payload);
            ...
            console.error('[reservation] Erreur réservation :', error);
            ...
            submitButton.textContent = config.texts.submitBooking;
        }
    });
});
