...
import { getTodayIsoString, normalizeDate, parseCleanIso } from './date-utils.js';
...
export function createCalendar({
    ...
    function getBackgroundPeriodEvents(fetchInfo) {
        ...
        if (state.selectedPeriodPreviewEvent) events.push(state.selectedPeriodPreviewEvent);
        return events;
    }

    function selectionAllowed(info) {
        let curr = new Date(info.start);
        ...
        while (curr <= end) {
            ...
            const m = String(curr.getMonth() + 1).padStart(2, '0');
            const d = String(curr.getDate()).padStart(2, '0');
            if (isDateBlocked(`${y}-${m}-${d}`)) return false;
            curr.setDate(curr.getDate() + 1);
        }
        ...
        if (state.bookingMode === 'period') return false;
        if (state.allDayBlockedDates.has(startDate)) return false;
        if (eventService.selectionOverlapsExistingEvent(info)) return false;

        const startHour = info.start.getHours() + info.start.getMinutes() / 60;
        const endHour = info.end.getHours() + info.end.getMinutes() / 60;
        if (startHour < 8 || endHour > 21) return false;
    ...
    calendar = new Calendar(calendarEl, {
        ...
        events: async (fetchInfo, successCallback, failureCallback) => {
            ...
            try {
                ...
                console.error('[reservation] Erreur chargement calendrier :', error);
        ...
        dateClick: async (info) => {
            ...
            if (state.bookingMode !== 'period' || calendar.view.type !== 'timeGridWeek') return;
